...
CREATE OR REPLACE FUNCTION ambil_data_pemesanan_tiket()
RETURNS TABLE(
    pengunjung_id INT,
    pengunjung_nama VARCHAR(50),
    pengunjung_saldo DECIMAL(10,2),
    wisata_id INT,
    wisata_nama VARCHAR(50),
    wisata_kuota INT,
    wisata_harga DECIMAL(10,2),
    wisata_jumlah_pembelian INT,
    beli_tiket INT,
    waktu_pesan TIMESTAMP
) AS
$$
DECLARE
    v_pengunjung_id INT;
    v_pengunjung_nama VARCHAR(50);
    v_pengunjung_saldo DECIMAL(10,2);
    
    v_wisata_id INT;
    v_wisata_nama VARCHAR(50);
    v_wisata_kuota INT;
    v_wisata_harga DECIMAL(10,2);
    v_wisata_jumlah_pembelian INT;

    v_beli_tiket INT;
    v_waktu_pesan TIMESTAMP;

    pengunjung_cursor CURSOR FOR
        SELECT p.id_pengunjung, p.nama, p.saldo FROM pemesanan_tiket.pengunjung AS p;
    
    destinasi_wisata_cursor CURSOR FOR
        SELECT d.id_wisata, d.nama, d.kuota, d.harga, d.jumlah_pembelian FROM pemesanan_tiket.destinasi_wisata AS d;
    
    pengunjung_wisata_cursor CURSOR FOR
        SELECT pw.id_pengunjung, pw.id_wisata, pw.beli_tiket, pw.waktu_pesan FROM pemesanan_tiket.pengunjung_wisata AS pw;
BEGIN
    -- Loop untuk data pengunjung
    OPEN pengunjung_cursor;
    LOOP
        FETCH pengunjung_cursor INTO v_pengunjung_id, v_pengunjung_nama, v_pengunjung_saldo;
        EXIT WHEN NOT FOUND;
        
        -- Loop untuk data destinasi wisata
        OPEN destinasi_wisata_cursor;
        LOOP
            FETCH destinasi_wisata_cursor INTO v_wisata_id, v_wisata_nama, v_wisata_kuota, v_wisata_harga, v_wisata_jumlah_pembelian;
            EXIT WHEN NOT FOUND;
            
            -- Loop untuk data pembelian tiket
            OPEN pengunjung_wisata_cursor;
            LOOP
                FETCH pengunjung_wisata_cursor INTO v_pengunjung_id, v_wisata_id, v_beli_tiket, v_waktu_pesan;
                EXIT WHEN NOT FOUND;

                -- Mengembalikan data sebagai output
                RETURN QUERY SELECT 
                    v_pengunjung_id, 
                    v_pengunjung_nama, 
                    v_pengunjung_saldo,
                    ...
                    v_wisata_kuota, 
                    v_wisata_harga, 
                    v_wisata_jumlah_pembelian,
                    v_beli_tiket, 
                    v_waktu_pesan;
            END LOOP;
            CLOSE pengunjung_wisata_cursor;
        END LOOP;
        CLOSE destinasi_wisata_cursor;
    END LOOP;
    CLOSE pengunjung_cursor;
END;
$$ LANGUAGE plpgsql;

SELECT ambil_data_pemesanan_tiket();
