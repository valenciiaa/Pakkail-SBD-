...
SELECT pengunjung.nama AS nama_pengunjung, destinasi_wisata.nama AS wisata, destinasi_wisata.harga, pengunjung.beli_tiket 
FROM 
...
DECLARE
    ...
    kuota_wisata INT;
BEGIN
    -- Mencari ID pengunjung dan saldo
    SELECT id_pengunjung, saldo INTO p_id_pengunjung, saldo_pengunjung
    ...
    -- Menambahkan jumlah tiket yang dibeli
    UPDATE pemesanan_tiket.pengunjung
    SET beli_tiket = COALESCE(beli_tiket, 0) + jumlah_tiket
    WHERE id_pengunjung = p_id_pengunjung;
END;
$$;
...
SELECT * FROM pemesanan_tiket.pengunjung WHERE nama = 'Tatayan';
